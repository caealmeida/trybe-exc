...
// com a classe 'tech';
input.addEventListener("input", changeInnerText);

function changeInnerText(event) {
	const getTech = document.querySelector(".tech");
	getTech.innerText = event.target.value;
}
...
function redirectToPortfolio() {
	window.location.replace("https://caealmeida.github.io");
}
...
myWebpage.addEventListener("mouseout", uncolorText);

function colorText(event) {
	event.target.style.color = "#2fc18c";
}

function uncolorText(event) {
